...
class Weatherinfo(Thread):
    ...
    def to_csv2(self, csv_file=None):
        '''
            This method is to get current weather information from Open Weather API
        ...
        if response is not None:
            ...
            with open(filename, 'w', newline='') as csv_stream:
                ...
                for key in csv_keys:
                    if key is 'main':
                        csv_values.append(float(data['main']['temp'])-273.15)
                        csv_values.append(float(data['main']['temp_min'])-273.15)
                        csv_values.append(float(data['main']['temp_max'])-273.15)
                        csv_values.append(data['main']['pressure'])
                        csv_values.append(0)
                        csv_values.append(0)
                        csv_values.append(data['main']['humidity'])
                        csv_values.append(0)
                    elif key is 'weather':
                        csv_values.append(data['weather'][0]['id'])
                        csv_values.append(data['weather'][0]['main'])
                        csv_values.append(data['weather'][0]['description'])
                        csv_values.append(data['weather'][0]['icon'])
                    elif key is 'clouds':
                        csv_values.append(data['clouds']['all'])
                    elif key is 'wind':
                        csv_values.append(data['wind']['speed'])
                        if 'deg' in data['wind'].keys():
                            csv_values.append(data['wind']['deg'])
                        else:
                            csv_values.append(0)
                    elif key is 'rain':
                        if 'rain' in data.keys() and len(data[key]) == 1:
                            csv_values.append(data[key]['3h'])
                        else:
                            csv_values.append(0)
                    elif key is 'sys':
                        csv_values.append(0)
                    elif 'dt_txt' in key:
                        csv_values.append(dateStr)
                    else:
                        csv_values.append(data[key])
                try:
